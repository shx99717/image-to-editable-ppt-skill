...
1. **작업 디렉터리 생성 및 입력 정규화**: 독립 작업 디렉터리를 만들고 입력(이미지/PDF/이미지 기반 PPT)을 `pages/page_NNN/source.png`로 정규화한 뒤, 부모 agent가 구체적 이미지 backend(`GenerateImage` / `image_gen.imagegen` / `codex-gpt-image` / CLI 등)를 조사·잠근 다음 이번 실행에 기록합니다.
...
- 이 skill은 입력 페이지를 편집 가능하게 재구성하는 용도이며, 처음부터 전체 PPT 콘텐츠를 생성하지 않습니다. 그 역할은 [codex-ppt-skill](https://github.com/shx99717/codex-ppt-skill)이 담당합니다.
